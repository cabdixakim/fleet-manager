...
- **Financial Statements Hub:** Full QuickBooks-like reporting in `FinancialStatements.tsx` with 7 tabs: P&L (with % of revenue column), Balance Sheet, Trial Balance, AR Aging (0–30/31–60/61–90/90+), AP Aging, Cash Flow Statement, and Expense Breakdown by account. All period-aware with preset buttons (This Month / Last Month / This Quarter / YTD / Last Year / Custom), Export and Print per report. Backend endpoints: `GET /api/gl/reports/ar-aging`, `/ap-aging`, `/cash-flow`, `/expense-breakdown`.
...
- **Petty Cash Cashbook:** Real petty cash account with live balance. Top-up from bank posts Dr Petty Cash (1003) / Cr Bank (1002). Expenses paid from petty cash deduct from balance and post Dr Expense / Cr Petty Cash automatically. Zero-config safe: if petty cash account hasn't been initialized, expenses with `paymentMethod=petty_cash` record normally without touching the petty cash page.
- **Multi-Bank Accounts:** `bank_accounts` table stores each configured bank (name, bank name, account number, GL code). Each bank gets its own GL account in range 1010–1099. A default bank account (GL 1002) is seeded on startup. New banks are created via `/api/bank-accounts` POST, which auto-seeds the GL account. Expense forms show a bank selector when payment method is "bank_transfer". GL posting uses `resolveBankGlCode(paymentMethod, bankAccountId)` to route to the correct bank's GL code. Pages: `/bank-accounts` (configuration) and `/bank-accounts/:id/reconcile`.
- **Bank Reconciliation:** `bank_reconciliation_items` table stores which GL entry lines have been cleared (ticked as matching bank statement). Reconciliation page shows all GL transactions for a bank's account, cleared balance vs GL balance vs user-entered statement balance, variance indicator. Mark individual transactions cleared/uncleared, bulk mark all, save changes.
- **GL Account additions:** 1003 Petty Cash (asset), 2050 Supplier Payables (liability) seeded on startup.
- **Clearance Agency AP integration:** Company Settings holds an `activeClearanceAgencyId` (FK → suppliers). When a trip reaches `at_zambia_entry`, the T1 fee is auto-created with `paymentMethod=fuel_credit` and linked to the active agency, posting Dr Clearance (5004) / Cr Supplier Payables (2050) and incrementing the supplier balance. Switching agencies mid-stream only affects future trips. Fee amount is still snapshotted at trip time (no retroactive repricing).
- **Payment method selector on all expense forms:** Finance, TripDetail, and TruckDetail expense forms all include a "Paid via" dropdown (Cash / Petty Cash / Fuel Credit / Bank Transfer). Selecting Fuel Credit reveals a supplier picker. GL posts to the correct account automatically.
...
- **Insurance Claims:** Full CRUD for insurance claims at `/insurance-claims` (sidebar: Fleet section). Fields: claimType (cargo_loss/accident/theft/third_party/other), status (draft/filed/acknowledged/approved/rejected/settled), insurer, policy, amounts, incident/filed/settled dates, description, notes. Linked to truck and optionally trip. Schema: `insurance_claims`.
- **Truck Insurance Policy:** Trucks now store insurer name, policy number, coverage amount, and expiry date. Editable via Fleet edit dialog. TruckDetail shows an insurance card with expiry highlighting (red if expired) and a link to Insurance Claims.
- **Broker Fee on InvoiceDetail:** When a batch has an agent linked, InvoiceDetail shows a Broker card at the bottom with broker name, fee/MT, delivered qty, and computed total broker fee (internal, not charged to client).
- **Document Vault:** Truck and driver document storage with expiry tracking. Each truck/driver profile has a Documents tab. Supported doc types: C29, White Book, Insurance, Road Tax, Fitness, Tare Cert, Route Permit, Customs Bond (trucks); Licence, Passport, Medical, Work Permit, Driver Card, NRC (drivers). Schema: `documents`.
- **Document Expiry Alerts:** Automatic bell notifications for documents expiring within 30 days. Runs on the same alert engine as overdue invoices and stuck trips. Notifies operations, manager, admin, and owner roles. Alert type: `doc_expiry` (amber).
- **Team Chat:** Real-time-like messaging with 5-second polling. Channels: `#general`, `#operations`, `#accounts` (seeded on startup). Trip-specific discussion threads auto-created on first message per trip and visible in TripDetail's Discussion tab. Chat page at `/chat` accessible to all roles. Schema: `chat_channels`, `chat_messages`. Users can delete their own messages.
